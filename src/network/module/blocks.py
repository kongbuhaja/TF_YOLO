...
class C3k(C3):
    def __init__(self, in_ch, out_ch, r=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        ch = int(out_ch * e)
        self.block = SequentialLayer([Bottleneck(ch, ch, shortcut, g, k=(k, k), e=1.0) for _ in range(r)])
...
class C2PSA(Layer):
    def __init__(self, in_ch, out_ch, r=1, dim=64, e=0.5):
        ...
        self.blocks = SequentialLayer([PSABlock(self.ch, self.ch // dim, 0.5) for _ in range(r)])
